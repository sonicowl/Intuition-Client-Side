...
function XmlParser:ParseXmlText(xmlText)
	-- REMOVE declaration
  local declaration = string.match(xmlText, "^<\?.*\?>(.*)")
  if declaration then xmlText = declaration end
...
function XmlParser:XmlValue(xmlTree, nodeName, default)
	for i,xmlNode in pairs(xmlTree.ChildNodes) do
		if(xmlNode.Name==nodeName) then
			local val = xmlNode.Value
...
function XmlParser:XmlNodes(xmlTree, nodeName)
	for i,xmlNode in pairs(xmlTree.ChildNodes) do
		if(xmlNode.Name==nodeName) then
...
function XmlParser:XmlAttributes(xmlTree, nodeName)
	for i = 1, #xmlTree.ChildNodes do
		if(xmlTree.ChildNodes[i].Name == nodeName) then
			return xmlTree.ChildNodes[i].Attributes
		end
	end
end